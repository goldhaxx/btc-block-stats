...
# Set up logging
logging.basicConfig(
    filename='./logs/block_stats_manual.log',
...
# Main function to pull data from a manually defined block range
def collect_block_data(start_block, end_block, rpc_url, auth_token=None):
    conn = create_connection()
    cursor = conn.cursor()

    try:
        logging.info(f"Starting block fetch from {start_block} to {end_block}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Fetch and store block stats for a specified range of blocks.')
    parser.add_argument('--start_block', type=int, help='The starting block height.')
    parser.add_argument('--end_block', type=int, help='The ending block height.')

    args = parser.parse_args()

    if args.start_block is None or args.end_block is None:
        print("Error: You must provide both --start_block and --end_block.")
        exit(1)

    rpc_url = RPC_URL_1
    auth_token = RPC_URL_1_API_KEY if RPC_URL_1_API_KEY else None  # Use None if no auth token is required

    collect_block_data(args.start_block, args.end_block, rpc_url, auth_token)
